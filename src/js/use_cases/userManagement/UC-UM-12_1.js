...
export const getSingleRouteLoginFlowPolicy = new Trend('Get Single Dynamic Route Login Flow Concurrent Users');
export const getSingleRouteClientSecretFlowPolicy = new Trend('Get Single Dynamic Route Client Secret Flow Concurrent Users');
export const getAllRoutesLoginFlowPolicy = new Trend('Get All Dynamic Routes Login Flow Concurrent Users');
export const getAllRoutesClientSecretFlowPolicy = new Trend('Get All Dynamic Routes Client Secret Flow Concurrent Users');
...
function generateOptions(){
  let f = JSON.parse(open('../../../resources/config/userManagement/UC-UM-12_1.options.json'));
  return new Array(f);
}
...
const VUS_DYNAMIC = 5;
const VUS_CREATE = 5;
const VUS_APP = sharedOptionsFile[0].scenarios.concurrent_getAppsWithJSession.vus;
const VUS_ROLES = sharedOptionsFile[0].scenarios.concurrent_getRolesWithJSession.vus;
const VUS_COMP = sharedOptionsFile[0].scenarios.concurrent_getComponentsWithJSession.vus;
const VUS_GROUP = sharedOptionsFile[0].scenarios.concurrent_getGroupsWithJSession.vus;
const VUS_TENANT = sharedOptionsFile[0].scenarios.concurrent_getTenantsWithJSession.vus;
const VUS_BRO = sharedOptionsFile[0].scenarios.concurrent_getBROStatusWithJSession.vus;
const ITERATIONS = sharedOptionsFile[0].scenarios.concurrent_getUsersWithJSession.iterations;
...
export function setupEnv() {
  ...
  group("Create Users to load DB", function () {
    for (let i = 0; i < 2*VUS_CREATE*(ITERATIONS+1); ++i){
  ...
  group("Creating Dynamic Routes",function(){
    ...
    for (let i = 0; i < 2*VUS_DYNAMIC*ITERATIONS; ++i){
...
export function teardownEnv(data) {
  ...
  group("Delete Dynamic Routes", function () {
    ...
    //for (let i = 0; i < VUS; ++i){
    for (let i = 0; i < VUS_DYNAMIC*ITERATIONS; ++i){
      //Delete Dynamic Routes Pre-Populated in DB for Login Flow
      //They are 2*VUS but From VUS to 2*VUS Routes are deleted by Delete Dynamic Route for Login Flow Scenario
  ...
  group("Delete users created in Create Use Case", function () {
    for (let i = 0; i < 2*VUS_CREATE*ITERATIONS; ++i) {
...
export function getUsersWithJSessionEnv(data) {
  ...
  let iteration = Math.trunc(iter/VUS);
  let new_iter = iter - iteration*VUS;

  let response;
  let jSessionId = data[2][new_iter];
  let user = getNewUser(data[1]+new_iter);
  let newUser = create_user(user);
...
export function getUsersWithClientSecretEnv(data) {
  ...
  let iteration = Math.trunc(iter/VUS);
  let new_iter = iter - iteration*VUS;
  let tokenSecret = data[4][new_iter];
  let refreshToken = data[5][new_iter];
...
export function getAppsWithJSessionEnv(data) {
  ...
  let response;
  let iter = scenario.iterationInTest;
  let iteration = Math.trunc(iter/VUS_APP);
  let new_iter = iter - iteration*VUS_APP;
  let jSessionId = data[2][new_iter];
  let user = getNewUser(data[1]+new_iter);
  let newUser = create_user(user);
...
export function createUsersWithJSessionEnv(data) {
  let response;
  let iter = scenario.iterationInTest;
  let iteration = Math.trunc(iter/VUS_CREATE);
  let new_iter = iter - iteration*VUS_CREATE;
  let jSessionId = data[2][new_iter];
...
export function createUsersWithClientSecretEnv(data) {
  ...
  let iteration = Math.trunc(iter/VUS_CREATE);
  let new_iter = iter - iteration*VUS_CREATE;
  let gap = VUS_CREATE*ITERATIONS + iter;
  let tokenSecret = data[4][new_iter];
  let refreshToken = data[5][new_iter];
...
export function updateUsersWithJSessionEnv(data) {
  ...
  let new_iter = iter - iteration*VUS_CREATE;
  let jSessionId = data[2][new_iter];
  let user = getNewUser(data[9]+new_iter);
  let newUser = create_user(user);
...
export function deleteUsersWithJSessionEnv(data) {
 ...
 // let gap = 2*VUS_CREATE + iter;
  let gap = 2*VUS_CREATE*iter_index + new_iter;
  let jSessionId = data[2][new_iter];
  let user = getNewUser(data[9]+gap);
  let newUser = create_user(user);
...
export function deleteUsersWithClientSecretEnv(data) {
  ...
  let gap = VUS_CREATE*(2*iter_index+1) + new_iter;
  let tokenSecret = data[4][new_iter];
  let refreshToken = data[5][new_iter];
  let user = getNewUser(data[9]+gap);
  let newUser = create_user(user);
...
export function getRolesWithJSessionEnv(data) {
  let response;
  let iter = scenario.iterationInTest;
  let iteration = Math.trunc(iter/VUS_ROLES);
  let new_iter = iter - iteration*VUS_ROLES;
  let jSessionId = data[2][new_iter];
  let user = getNewUser(data[1]+new_iter);
  let newUser = create_user(user);
...
export function getComponentsWithJSessionEnv(data) {
  let response;
  let iter = scenario.iterationInTest;
  let iteration = Math.trunc(iter/VUS_COMP);
  let new_iter = iter - iteration*VUS_COMP;
  let jSessionId = data[2][new_iter];
  let user = getNewUser(data[1]+new_iter);
  let newUser = create_user(user);
...
export function getGroupsWithJSessionEnv(data) {
  let response;
  let iter = scenario.iterationInTest;
  let iteration = Math.trunc(iter/VUS_GROUP);
  let new_iter = iter - iteration*VUS_GROUP;
  let jSessionId = data[2][new_iter];
  let user = getNewUser(data[1]+new_iter);
  let newUser = create_user(user);
...
export function getTenantsWithJSessionEnv(data) {
  let response;
  let iter = scenario.iterationInTest;
  let iteration = Math.trunc(iter/VUS_TENANT);
  let new_iter = iter - iteration*VUS_TENANT;
  let jSessionId = data[2][new_iter];
  let user = getNewUser(data[1]+new_iter);
  let newUser = create_user(user);
...
export function getBROStatusWithJSessionEnv(data) {
  let response;
  let iter = scenario.iterationInTest;
  let iteration = Math.trunc(iter/VUS_BRO);
  let new_iter = iter - iteration*VUS_BRO;
  let jSessionId = data[2][new_iter];
...
export function getSingleRouteWithJSessionEnv(data) {
  let response;
  let iter = scenario.iterationInTest;
  let iteration = Math.trunc(iter/VUS_DYNAMIC);
  let new_iter = iter - iteration*VUS_DYNAMIC;
  let jSessionId = data[2][new_iter];
  let gap = VUS_DYNAMIC + new_iter;
...
export function updateDynamicRoutesWithJSessionEnv(data) {
  ...
  group("Use Case: Concurrent Users - Update Dynamic Routes with Login Auth Flow", function () {
    ...
    group("Update Dynamic Routes", function() {
      dynamicRoutes['id'] = data[6]+new_iter;
...
export function updateDynamicRoutesWithClientSecretEnv(data) {
  ...
  group("Use Case: Concurrent Users - Update Dynamic Routes with Client Secret Auth Flow", function () {
    ...
    group("Update Dynamic Routes", function() {
      dynamicRoutes['id'] = data[8]+new_iter;
      dynamicRoutes['predicates'][0]['args']= predToUpdate;
...
export function deleteDynamicRoutesWithJSessionEnv(data) {
  ...
  let gap = VUS_DYNAMIC*ITERATIONS + iter;
